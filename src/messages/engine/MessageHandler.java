...
public class MessageHandler {
	...
	public void handleMessage(ByteBuffer buffer,NioChannel channel) throws Exception{
		...
		System.out.println("remaning :"+ buffer.remaining());
		...
		switch(messageID){
			case 0 : //Reception simple d'un message
				if (length > buffer.remaining()) {
					NioChannel.BufferState state = channel.new BufferState(length, length
							- buffer.remaining());
					channel.getReceiveBuffer().put(buffer); // Stores the partial message in a Buffer
				} else {
					byte[] deliver_array = new byte[length-14];
					...
					if(checkMessage(deliver_array,checksum)){
						...
						Message m2 = new AckMessage(lamport_timestamp,sender_id,ack_payload.array());
			 ...
			 case 3 : 
				 //Réception d'un BrodcastJoin : Au moment du deliver on bloque le Peer.
				 	 id_sender = buffer.getInt();
				 	 lamport_timestamp = buffer.getInt();
